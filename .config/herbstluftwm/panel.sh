...

{
    ...
    while true ; do
        ...
        printf 'date\t^fg(#d3cab3)%(%I:%M %p)T^fg(#909090), %(%Y-%m)T-^fg(#d3cab3)%(%d)T\n'
        sleep 1 || break
    ...
    while true ; do
        ...
        echo -n "$separator"
        echo -n "^bg()^fg(#d3cab3) ${windowtitle//^/^^}"
        # small adjustments
        battery=$(acpi | sed -n '2 p')

		wifiname=$(nmcli -t -f name connection show --active | sed -n '1 p')

        right="$separator^bg()^fg(#d3cab3) $battery^bg() $separator^bg()^fg(#d3cab3) $wifiname $separator^bg()^fg() $date $separator"
