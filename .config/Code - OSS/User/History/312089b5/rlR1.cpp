...
void WindowManager::OnButtonPress(const XButtonEvent& e){
    ...
    drag_start_y_ = e.y_root; // save cursor's starting position
    ...
    drag_start_frame_x_ = x;
    drag_start_frame_y_ = y;

    drag_start_frame_width_ = width;
    drag_start_frame_height_ = height;
...
void WindowManager::OnMotionNotify(const XMotionEvent& e){
    ...
    const int deltaY = dragY - drag_start_y_;

    if (e.state & Button1Mask)
    // alt+lmouse = move window
    {
        int destFrameX = drag_start_frame_x_ + deltaX;
        int destFrameY = drag_start_frame_y_ + deltaY;
    ...
    {
        int sizeDeltaX = max(deltaX, -drag_start_frame_width_);
        int sizeDeltaY = max(deltaY, -drag_start_frame_height_);
    }
}
